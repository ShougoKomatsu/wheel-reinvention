MouseGesture（本ソフトウェア）は、小松翔悟（作者）が個人の趣味で制作したものであり、このソフトを使ったことにより発生した、いかなる損害に対しても、作者は責任を負いません。
また、作者は本ソフトウェアのソースコードに関する著作権は放棄していません。流用等の相談はGithubコミュニティから行ってください。


================================================================================
MICROSOFT FOUNDATION CLASS ライブラリ: MouseGesture プロジェクトの概要
===============================================================================

アプリケーション ウィザードが作成した MouseGesture には Microsoft 
Foundation Class の基本的な使い方が示されています。アプリケーション作成のひな型
としてお使いください。

このファイルには MouseGesture アプリケーションを構成している各ファイ
ルの概要説明が含まれます。

MouseGesture.vcxproj
...
MouseGesture.h
   ...
   CMouseGestureApp アプリケーション クラスを宣言します。

MouseGesture.cpp
   これは、アプリケーション クラス CMouseGestureApp を含むメイン アプリケー
   ション ソース ファイルです。
...
res\MouseGesture.ico
   これは、アプリケーションのアイコンとして使用されるアイコンファイルです。この
   アイコンはメイン リソース ファイル MouseGesture.rc に含まれていま
   す。

res\MouseGesture.rc2
...
MouseGestureDlg.h, MouseGestureDlg.cpp - ダイアログ
   これらのファイルは CMouseGestureDlg クラスを含みます。このクラスはアプ
   リケーションのメイン ダイアログの動作を定義します。ダイアログ テンプレートは
   Microsoft Visual C++ で編集可能な MouseGesture.rc に含まれます。
...
StdAfx.h, StdAfx.cpp
   ...
   MouseGesture.pch や既にコンパイルされた型のファイル StdAfx.obj を
   ビルドするために使用されます。
